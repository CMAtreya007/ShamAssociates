...
import asyncio
import logging
from app.models import Nifty50Daily, StockDetailDaily, IndexDaily, CorporateAction, CustomStockWatchlist
from app.database import AsyncSessionLocal
from app.config import settings
from app.services.nse_fetcher import NSEFetcher, classify_corporate_action, safe_float
...
def render_stock_detail_sheet(
    ...
    per_change_30d: Optional[float],
    per_change_365d: Optional[float],
    near_wkh: Optional[float],
    near_wkl: Optional[float],
    ...
    title_cell = ws_stock.cell(1, 1, f"{company_name or sym} ({sym}) — Complete Daily Analytics & Corporate Actions [{stock_date}]")
    ...
    price_rows = [
        ("Date", str(stock_date), "Symbol", str(sym)),
        ("Last Traded Price (LTP)", f"₹ {ltp:,.2f}" if ltp is not None else "-", "Day Open", f"₹ {open_val:,.2f}" if open_val is not None else "-"),
        ("Change", f"₹ {change:+,.2f}" if change is not None else "-", "Day High", f"₹ {high:,.2f}" if high is not None else "-"),
        ("% Change", f"{pct_change:+.2f}%" if pct_change is not None else "-", "Day Low", f"₹ {low:,.2f}" if low is not None else "-"),
        ("Previous Close", f"₹ {previous_close:,.2f}" if previous_close is not None else "-", "VWAP / Average Price", f"₹ {float(vwap_val):,.2f}" if vwap_val else "-"),
        ("52-Week High", f"₹ {year_high:,.2f}" if year_high is not None else "-", "52-Week Low", f"₹ {year_low:,.2f}" if year_low is not None else "-"),
        ("52W High Date", yr_high_dt, "52W Low Date", yr_low_dt),
        ("30-Day % Change", f"{per_change_30d:+.2f}%" if per_change_30d is not None else "-", "365-Day % Change", f"{per_change_365d:+.2f}%" if per_change_365d is not None else "-"),
        ("Distance to 52W High", f"{near_wkh:+.2f}%" if near_wkh is not None else "-", "Distance to 52W Low", f"{near_wkl:+.2f}%" if near_wkl is not None else "-"),
    ...
    ffmc_cr = (detail.free_float_mcap / 10000000.0) if (detail and detail.free_float_mcap) else ((ffmc / 10000000.0) if ffmc else None)
    vol_shares = volume or (detail.total_volume if detail else None) or t_info.get("totalTradedVolume") or t_info.get("quantitytraded")
    turnover_cr = (detail.total_turnover / 10000000.0) if (detail and detail.total_turnover) else ((turnover / 10000000.0) if turnover else None)

    trade_rows = [
        ("Date", str(stock_date), "Total Turnover (₹ Cr)", f"₹ {turnover_cr:,.2f} Cr" if turnover_cr is not None else "-"),
    ...
    is_fno = str(m_data.get("isFNOSec") or (series == "EQ")).capitalize()
    ...
    sec_rows = [
        ("Date", str(stock_date), "Series", str(series or "EQ")),
    ...
    # 4A. Corporate Actions & Dividends
    ws_stock.merge_cells(f"A{cur_row}:D{cur_row}")
...
async def build_nifty50_workbook(target_date: str, output_path: str) -> str:
    ...
    for stock in stocks:
        ...
        render_stock_detail_sheet(
            ws_stock=ws_stock,
            sym=sym,
            company_name=stock.company_name,
            stock_date=stock.date or target_date,
            ltp=stock.ltp,
            open_val=stock.open,
            high=stock.high,
            low=stock.low,
            ...
            per_change_30d=stock.per_change_30d,
            per_change_365d=stock.per_change_365d,
            near_wkh=stock.near_wkh,
            near_wkl=stock.near_wkl,
            volume=stock.volume,
...
async def build_custom_stocks_workbook(target_date: str, output_path: str, username: str = "admin") -> str:
    ...
    # If watchlist is empty, write an informational row and return
    if not symbols_list:
        empty_row = [target_date, "-", "No custom stocks added yet. Use 'Add Stock' on the dashboard to track stocks.", "-", None, None, None, None, None, None, None, None, None, None, None, None, None, None, "-", "-", None, None, None, None, None, None]
        ws_overview.append(empty_row)
        format_cell(ws_overview.cell(2, 1), font=MUTED_FONT, align=Alignment(horizontal="center"))
        format_cell(ws_overview.cell(2, 3), font=MUTED_FONT, align=Alignment(horizontal="left"))
    ...
    for s in stocks_meta:
        ...
        safe_sym = sym.replace("/", "-").replace("\\", "-").replace("?", "").replace("*", "").replace(":", "-").replace("[", "(").replace("]", ")")[:31]
        ws_stock = wb.create_sheet(title=safe_sym)
        ws_stock.sheet_properties.tabColor = "00B386"
        detail = s.get("detail")
        render_stock_detail_sheet(
            ws_stock=ws_stock,
            sym=sym,
            ...
            year_low=s["year_low"],
            per_change_30d=s["per_change_30d"],
            per_change_365d=s["per_change_365d"],
            near_wkh=s["near_wkh"],
            near_wkl=s["near_wkl"],
            volume=s["volume"],
            turnover=s["turnover"],
...
async def generate_full_export_bundle(target_date: Optional[str] = None, force_rebuild: bool = False, username: str = "admin") -> Tuple[str, List[str], str]:
    """Builds Nifty 50, Broad Market Indices, and Custom Stocks workbooks in a dated directory and packages them into a single ZIP.
    ...
    custom_file = export_dir / f"custom_stocks_{target_date}.xlsx"
    ...
    # Instant Cache Check: If pre-generated ZIP and all required workbooks exist, return immediately in < 10ms!
    if not force_rebuild and zip_file.exists() and nifty_file.exists() and indices_file.exists() and custom_file.exists() and zip_file.stat().st_size > 10000:
        files = [str(nifty_file), str(indices_file), str(custom_file), str(n50_master_path), str(idx_master_path)]
    ...
    await asyncio.gather(
        ...
        build_custom_stocks_workbook(target_date, str(custom_file), username=username),
    ...
    with zipfile.ZipFile(zip_file, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        if os.path.exists(custom_file):
            z.write(custom_file, arcname=custom_file.name)
